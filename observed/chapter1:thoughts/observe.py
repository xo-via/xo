...
Signal — this universe as a node among universes (see SIGNAL.md):
    ...
    observe.py run [INTERVAL] [LIFESPAN] [FATE]
                           run as a live instance: emit + receive until lifespan ends
    observe.py life             show lifespan or whether xo.json is sealed
"""
...
def ensure_running() -> None:
    ...
    # own comment, keep everything else, then install the current observe.py line.
    kept = [
...
def main(argv: list[str]) -> int:
    ...
    if cmd == "emit":
        if len(argv) < 3:
            print("usage: observe.py emit <kind> [body]", file=sys.stderr)
            return 2
    ...
    if cmd == "run":
        ...
        while i < len(args):
            ...
            if a.isdigit() and interval is None:
                ...
                print("usage: observe.py run [interval] [lifespan_seconds] [fate]",
                      file=sys.stderr)
